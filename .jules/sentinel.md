...
## 2026-06-19 - Discarded Cryptographic API Results
**Vulnerability:** The application used `_ =` to discard the `OSStatus` return value of `SecRandomCopyBytes` during encryption key generation. If random generation failed, the key could be populated with predictable zeros.
**Learning:** System cryptographic APIs must have their return values strictly checked to prevent silent security failures.
**Prevention:** Never discard the result of security/cryptographic APIs like `SecRandomCopyBytes`. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures.
