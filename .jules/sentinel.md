...
## 2026-08-04 - Ignored Cryptographic Return Value
**Vulnerability:** In `HiyoStore.swift`, the return value of `SecRandomCopyBytes` was being ignored during encryption key generation. If the random number generator failed, this could theoretically result in a zeroed or predictable key being used for encryption at rest.
**Learning:** System cryptographic APIs in Swift that return `OSStatus` must have their results explicitly checked, as failures might lead to insecure fallback states (like an empty byte array).
**Prevention:** Always verify the returned `OSStatus` against `errSecSuccess` and handle failures by throwing a domain-specific error (e.g., `SecurityError.encryptionFailed`) to prevent the application from proceeding in an insecure state.
