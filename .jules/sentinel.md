...
## 2026-05-12 - Discarded Cryptographic API Results
**Vulnerability:** System cryptographic and security APIs (like `SecRandomCopyBytes`) had their results discarded via the `_ =` operator, which silently ignores failures (such as returning an error code). In the concrete pattern used here (`Data(count: ...)`), the output buffer is zero-initialized, so if `SecRandomCopyBytes` fails the buffer remains unchanged (that is, still all zeroes), silently bypassing encryption mechanisms by using a known or predictable key.
**Learning:** Never ignore the return status of cryptographic functions. Failure to generate secure random bytes or derive keys must strictly halt execution or throw an explicit security error, regardless of whether the unchanged fallback bytes are zeroed or otherwise predictable.
**Prevention:** Always explicitly verify the returned `OSStatus` (e.g., `guard status == errSecSuccess else { throw ... }`) and handle failures appropriately instead of discarding the result to prevent silent security bypasses.
