...
## 2026-08-30 - Silent Cryptographic Failure via Discarded Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` when generating the main encryption key in `HiyoStore`. If entropy generation failed, it would silently leave the buffer zeroed, resulting in a completely predictable zero-key for data encryption.
**Learning:** System cryptographic APIs that return an `OSStatus` (like `errSecSuccess`) indicate success or failure. Discarding this status can lead to catastrophic security failures, such as using uninitialized or zeroed memory for critical secrets.
**Prevention:** Always explicitly check the `OSStatus` of cryptographic functions against `errSecSuccess`. If the operation fails, handle the error appropriately (e.g., throw an exception) to prevent the application from proceeding in an insecure state.
