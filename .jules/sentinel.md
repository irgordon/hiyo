...
## 2026-10-24 - Ineffective Symlink Validation Due to Path Resolution
**Vulnerability:** In `SecureMLX.swift`, the code attempted to prevent symlink-based sandbox escapes by checking `.isSymbolicLinkKey` on the components of a path that had already been resolved using `resolvingSymlinksInPath()`. Because the path was already resolved, any symlinks were removed before the check, making the validation useless and allowing symlink attacks.
**Learning:** Calling `resolvingSymlinksInPath()` removes all symlinks from the path. Verifying symlinks must be done on the unresolved path components. Furthermore, when ensuring path containment after resolving, both the target path and the base directory must be resolved before comparison because standard directories like `/var` are often symlinks.
**Prevention:** Check for symlinks on the unresolved path first. Then, resolve both the target path and the intended base directory, and ensure the resolved target path is strictly contained within the resolved base directory using exact match or a trailing slash prefix.
