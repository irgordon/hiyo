...
## 2026-05-25 - Ignored SecRandomCopyBytes Return Value
**Vulnerability:** The application invoked `SecRandomCopyBytes` to generate a cryptographic key in `HiyoStore.swift` but discarded the returned `OSStatus` using `_ =`. If random byte generation failed (e.g., due to entropy depletion), the key buffer remained as initialized (all zeros), silently leading to a predictable key.
**Learning:** System APIs that perform security-critical functions like random byte generation can fail. Ignoring their return results can lead to silent failures that completely bypass the intended security mechanisms, often resulting in known/zeroed outputs.
**Prevention:** Always capture and explicitly verify the `OSStatus` returned by cryptographic APIs like `SecRandomCopyBytes` (e.g., ensure it equals `errSecSuccess`). Throw domain-specific errors on failure to prevent the application from proceeding in an insecure state.
