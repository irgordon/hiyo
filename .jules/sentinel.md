...
## 2026-08-08 - Ignored Cryptographic Return Status
**Vulnerability:** In `HiyoStore.swift`, the return status (`OSStatus`) of `SecRandomCopyBytes` was ignored using `_ =`. If random number generation fails, it could silently produce a zeroed or uninitialized encryption key.
**Learning:** System cryptographic and security APIs must never have their results discarded. Ignoring failures can lead to insecure states, such as a predictable zeroed key, which defeats the purpose of encryption entirely.
**Prevention:** Always capture and explicitly verify the returned `OSStatus` against `errSecSuccess` (and handle failures by throwing an error or crashing) to prevent silent security bypasses.
