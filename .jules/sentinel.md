...
## 2026-07-22 - Unchecked Cryptographic API Return Values
**Vulnerability:** In `HiyoStore.swift`, the application used `_ = keyData.withUnsafeMutableBytes { SecRandomCopyBytes(...) }`, explicitly discarding the `OSStatus` return value. If random generation fails, it silently produces an all-zero encryption key.
**Learning:** System cryptographic APIs like `SecRandomCopyBytes` do not throw Swift errors; they return integer status codes (like `OSStatus`). Discarding these codes hides critical failures.
**Prevention:** Always verify the returned `OSStatus` from `SecRandomCopyBytes` against `errSecSuccess` and explicitly throw a domain-specific error if the operation fails. Never discard the result with `_ =`.
