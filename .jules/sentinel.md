...
## 2026-06-15 - Useless Symlink Validation on Resolved Paths
**Vulnerability:** Path validation to prevent symlink-based sandbox escapes called `resolvingSymlinksInPath()` before checking the `isSymbolicLinkKey` attribute. Because the path was already resolved, the symlink check always returned false, rendering the security control useless. Additionally, it did not verify that the resolved path remained within the intended base directory using a resolved base path.
**Learning:** `resolvingSymlinksInPath()` removes all symlinks from a path. Applying symlink checks on a resolved path will never detect symlinks. Furthermore, standard system directories (like `/var`) are often symlinks themselves, so containment checks must compare against a resolved base directory.
**Prevention:** Check for symlinks on the unresolved path components first. Then, resolve both the target path and the base directory, and explicitly verify that the resolved target is contained within the resolved base directory (using strict exact match or trailing slash prefix).
