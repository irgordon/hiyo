...
## 2024-07-21 - Unchecked Cryptographic RNG Failure
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =`, which could result in a silent failure where the encryption key remains initialized as all zeros if the system RNG fails.
**Learning:** System cryptographic APIs do not throw Swift errors; they return `OSStatus` codes. Ignoring these codes bypasses failure detection for critical security operations.
**Prevention:** Always verify that cryptographic operations return `errSecSuccess` (and explicitly handle failures by throwing errors) rather than discarding the result.
