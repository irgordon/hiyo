...
## 2024-07-10 - Unchecked Cryptographic RNG Failure
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using the `_ =` operator when generating a new encryption key. If the system RNG failed, it would silently result in an all-zero encryption key being used and stored in the keychain.
**Learning:** System cryptographic APIs in Swift often return `OSStatus` instead of throwing errors. Discarding these return values bypasses critical failure conditions.
**Prevention:** Always explicitly capture and verify the `OSStatus` returned by security functions (e.g., `guard status == errSecSuccess else { throw ... }`) rather than ignoring it.
