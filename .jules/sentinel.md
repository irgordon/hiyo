...
## 2026-06-02 - Unchecked Cryptographic API Results
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using the `_ =` operator in `HiyoStore.swift`. If random byte generation failed (e.g., due to entropy depletion), it would silently proceed with an all-zero key, severely compromising encryption.
**Learning:** System cryptographic APIs often return an `OSStatus` rather than throwing errors. Discarding these results can lead to silent failures that undermine security mechanisms.
**Prevention:** Always capture and explicitly verify the `OSStatus` from system security APIs (like `errSecSuccess`) and handle failures appropriately by throwing a domain-specific error to prevent insecure states.
