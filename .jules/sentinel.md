...
## 2026-05-30 - Unchecked Cryptographic API Result
**Vulnerability:** In `HiyoStore.swift`, the result of `SecRandomCopyBytes` was explicitly ignored using `_ =`. If random byte generation failed, it could result in the application using an all-zero encryption key without throwing an error or halting execution.
**Learning:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded via the `_ =` operator, as doing so will cause silent security bypasses (like zeroed encryption keys).
**Prevention:** Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately (e.g., throwing a domain-specific error such as `SecurityError.encryptionFailed`) to prevent insecure states.
