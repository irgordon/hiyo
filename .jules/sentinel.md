...
## 2026-06-21 - Unchecked Cryptographic RNG Failure
**Vulnerability:** The result of `SecRandomCopyBytes` was silently discarded using `_ =` in `HiyoStore.swift`. If the system random number generator failed, the encryption key would remain initialized to all zeros, leading to a predictable key and a complete compromise of the encrypted storage.
**Learning:** Security APIs in Swift that return an `OSStatus` do not throw exceptions. Their return values must be explicitly checked, especially when allocating cryptographic key material, otherwise failures fail open instead of failing closed.
**Prevention:** Always verify the returned `OSStatus` from system security and cryptographic APIs (like `errSecSuccess`) and throw domain-specific errors upon failure instead of discarding the result.
