...
## 2026-07-24 - Unchecked Cryptographic RNG
**Vulnerability:** The return value of `SecRandomCopyBytes` was discarded using `_ =` during encryption key generation. If random generation fails, the buffer remains zeroed, resulting in a completely predictable null-byte encryption key.
**Learning:** System cryptographic APIs must have their return values explicitly checked; ignoring them can silently bypass security mechanisms.
**Prevention:** Always verify the returned `OSStatus` against `errSecSuccess` and throw a domain-specific error upon failure to halt execution.
