...
## 2026-06-15 - Unhandled SecRandomCopyBytes Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` when generating an encryption key in `HiyoStore.swift`. This could lead to a silent security bypass where a zeroed key is used if random byte generation fails.
**Learning:** Cryptographic API failures must be handled explicitly. Discarding error codes from system APIs like `SecRandomCopyBytes` creates a false sense of security.
**Prevention:** Always verify that cryptographic operations return `errSecSuccess` and throw a domain-specific error (like `SecurityError.encryptionFailed`) if they fail. Do not use the `_ =` operator on these critical functions.
