...
## 2024-05-24 - Unhandled Cryptographic API Results
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` in `HiyoStore.swift`. If random byte generation failed, the application would silently proceed with a zero-filled (uninitialized) `Data` buffer as the encryption key, completely undermining the encryption scheme.
**Learning:** Cryptographic functions on Apple platforms often return an `OSStatus` rather than throwing Swift errors. Discarding these results can lead to silent failures that result in catastrophic security degradation (e.g., all zero keys).
**Prevention:** System cryptographic APIs must never have their results discarded. Always capture the `OSStatus`, explicitly verify it against `errSecSuccess`, and throw a domain-specific error on failure.
