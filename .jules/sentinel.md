...
## 2026-05-20 - Unchecked Cryptographic API Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` when generating the master encryption key. If random byte generation fails, it returns an error code but leaves the provided buffer unmodified (potentially all zeros). This could lead to the encryption key being a predictable, weak key.
**Learning:** System cryptographic APIs often return status codes rather than throwing exceptions or returning optionals. Discarding these status codes can lead to silent security failures.
**Prevention:** Always capture and verify the `OSStatus` result of system cryptographic APIs (like `SecRandomCopyBytes`). Ensure it equals `errSecSuccess` before proceeding to use the generated or processed data. Handle failures by throwing a domain-specific error.
