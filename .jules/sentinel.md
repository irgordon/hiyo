...
## 2026-06-15 - Unhandled Cryptographic RNG Failures
**Vulnerability:** The application used `SecRandomCopyBytes` to generate an encryption key but discarded its return status with `_ =`. If the system RNG failed, it resulted in a silently zeroed `Data` buffer being used as an encryption key.
**Learning:** System cryptographic APIs like `SecRandomCopyBytes` can fail under certain OS conditions. If their results are discarded instead of checked, failures result in weak or zeroed cryptographic keys instead of safe crashes or caught errors.
**Prevention:** System cryptographic and security APIs must never have their results discarded via the `_ =` operator. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately to prevent silent security bypasses.
