...
## 2024-05-23 - Unchecked Cryptographic RNG Status
**Vulnerability:** The application used `SecRandomCopyBytes` to generate encryption keys but discarded the return status using `_ =`. If the random number generator failed, the buffer would remain zeroes, resulting in a predictable "zero key" being used for encryption.
**Learning:** System APIs that generate random bytes can fail. Discarding their return status silences these failures and leads to silent fallback to insecure defaults.
**Prevention:** Always verify the returned `OSStatus` of `SecRandomCopyBytes` against `errSecSuccess` and handle failures appropriately.
