...
## 2026-05-13 - Ignored Cryptographic API Return Values
**Vulnerability:** System cryptographic APIs like `SecRandomCopyBytes` can fail under certain conditions (e.g., lack of entropy, sandboxing issues). In `HiyoStore.swift`, the returned `OSStatus` was discarded using the `_ =` operator. This could lead to a silent security bypass where a zeroed encryption key buffer is used if entropy generation fails.
**Learning:** Returning success does not guarantee cryptographic operations succeeded if the result code isn't explicitly validated. Discarding error codes from system security APIs bypasses their intended safety mechanisms.
**Prevention:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded via the `_ =` operator. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately to prevent silent security bypasses.
