...
## 2026-07-28 - Unverified Cryptographic Random Generation
**Vulnerability:** The return status of `SecRandomCopyBytes` was discarded using `_ =` when generating an encryption key in `HiyoStore.swift`. If the random number generator failed, the system could silently proceed with a zeroed or predictable key, compromising the encryption of all exported chats.
**Learning:** System APIs that generate secure random data or perform critical cryptographic operations must always have their result codes explicitly verified. Failure conditions can happen (e.g., entropy pool depletion, OS errors) and ignoring them bypasses the security guarantee.
**Prevention:** Always verify that `OSStatus` returned by `SecRandomCopyBytes` equals `errSecSuccess`. If it fails, throw a domain-specific error (like `SecurityError.encryptionFailed`) to halt the operation safely rather than continuing with insecure state.
