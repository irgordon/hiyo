...
## 2024-05-24 - Unchecked SecRandomCopyBytes Return Value
**Vulnerability:** The return value of `SecRandomCopyBytes` was being discarded using `_ =`, meaning if random byte generation failed, an uninitialized (zeroed) buffer would be used as a cryptographic key.
**Learning:** Cryptographic system calls can fail under low-resource or sandbox constraints. Discarding their return status leads to silent, catastrophic security failures rather than obvious crashes.
**Prevention:** Always verify the returned `OSStatus` of `SecRandomCopyBytes` is exactly `errSecSuccess` before using the generated bytes, throwing a specific error on failure.
