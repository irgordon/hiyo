...
## 2026-05-17 - Unchecked SecRandomCopyBytes Return Value
**Vulnerability:** The application used `SecRandomCopyBytes` for generating secure encryption keys but silently ignored the returned `OSStatus` using `_ =`, or threw a catchable error. This could allow the application to proceed with a predictable or all-zero key if random number generation fails.
**Learning:** System cryptographic and security APIs must never have their results discarded. A failure in key generation is a critical security violation that compromises all subsequent operations relying on that key.
**Prevention:** Always explicitly verify the returned `OSStatus` is `errSecSuccess`. When critical cryptographic operations fail, use `fatalError` to safely crash the application and prevent it from entering an insecure state, rather than silently ignoring the failure or throwing catchable errors.
