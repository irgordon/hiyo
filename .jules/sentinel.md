...
## 2026-08-12 - Unchecked Cryptographic Function Return Values
**Vulnerability:** System cryptographic APIs like `SecRandomCopyBytes` can fail. If the return value (`OSStatus`) is ignored (e.g., using `_ =`), failures go undetected, potentially resulting in zeroed encryption keys and complete loss of confidentiality.
**Learning:** Silent failures in cryptographic generation bypass intended security mechanisms, often leading to fallback states that use all-zero or uninitialized memory.
**Prevention:** Always explicitly capture and verify the `OSStatus` of cryptographic operations (ensuring it equals `errSecSuccess`). Throw domain-specific errors immediately upon failure to halt execution.
