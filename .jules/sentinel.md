...
## 2026-08-18 - Unchecked Cryptographic Randomness Generation
**Vulnerability:** The application discarded the `OSStatus` result of `SecRandomCopyBytes` when generating the master encryption key. If the system's random number generator failed, the key buffer would remain initialized to all zeros, resulting in a predictable "zero key" being used and stored in the Keychain.
**Learning:** System cryptographic APIs in Swift do not throw exceptions on failure. Discarding their return values can lead to silent failures where security-critical operations proceed with predictable or empty data.
**Prevention:** Always explicitly capture and verify the `OSStatus` of Security framework functions against `errSecSuccess`. Never use `_ =` to silence compiler warnings for these APIs.
