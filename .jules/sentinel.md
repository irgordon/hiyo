...
## 2026-05-24 - Unchecked Random Byte Generation
**Vulnerability:** In `HiyoStore.swift`, the result of `SecRandomCopyBytes` was explicitly ignored using `_ =`. If random byte generation failed, the application would silently proceed with a zero-filled `Data` buffer, resulting in a predictable all-zero encryption key.
**Learning:** System cryptographic APIs in Swift often return `OSStatus` rather than throwing errors. Ignoring these return values with `_ =` masks critical failures (like entropy depletion) and can silently compromise cryptographic guarantees.
**Prevention:** Always capture and verify the `OSStatus` returned by security APIs against `errSecSuccess`. If the operation fails, throw a domain-specific error to halt execution rather than continuing with insecure default states.
