...
## 2026-05-28 - Unchecked Cryptographic OSStatus
**Vulnerability:** The OSStatus returned by `SecRandomCopyBytes` was discarded using `_ =` during encryption key generation in `HiyoStore`. If key generation failed (e.g., entropy pool exhausted), the error was ignored, resulting in a predictable (zeroed) encryption key.
**Learning:** System cryptographic APIs do not throw standard Swift errors. They return integer error codes (`OSStatus`) that must be explicitly checked, otherwise security failures will silently compromise the system.
**Prevention:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded via the `_ =` operator. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and throw a domain-specific error (like `SecurityError.encryptionFailed`) upon failure.
