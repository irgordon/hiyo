...
## 2026-08-22 - Unchecked Cryptographic RNG Failure
**Vulnerability:** `SecRandomCopyBytes` results were being discarded with `_ =`, which could result in a silent failure leaving the encryption key as all zeros (a weak, predictable key) if the system random number generator failed.
**Learning:** System cryptographic and security APIs can fail under resource constraints or sandbox issues. Ignoring their return values can lead to devastating security bypasses like zeroed keys.
**Prevention:** Always explicitly verify the returned `OSStatus` from APIs like `SecRandomCopyBytes` is `errSecSuccess` and throw an appropriate error to halt the operation on failure.
