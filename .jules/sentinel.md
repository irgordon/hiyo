...
## 2026-06-26 - Unchecked Cryptographic Random Number Generation
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` during encryption key generation in `HiyoStore.swift`.
**Learning:** System security APIs can fail (e.g., entropy pool depletion). Discarding their return status can lead to silent failures, potentially resulting in zeroed/predictable encryption keys.
**Prevention:** Always verify the `OSStatus` of cryptographic operations (like `errSecSuccess`) and throw/handle an error appropriately to prevent generating weak keys.
