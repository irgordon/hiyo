...
## 2026-06-05 - Ineffective Symlink Validation
**Vulnerability:** In `SecureMLX.swift`, the code called `resolvingSymlinksInPath()` on a URL *before* checking for `isSymbolicLinkKey`. Because `resolvingSymlinksInPath()` removes all symlinks by resolving them to their final destinations, the subsequent check for `isSymbolicLinkKey` would always be false, completely bypassing the sandbox escape protection.
**Learning:** Checking for symlinks on a path that has already been resolved is useless. The check must be performed on the raw, unresolved path.
**Prevention:** Always check for `isSymbolicLinkKey` on the unresolved path components, and additionally verify that the final resolved path is still contained within the expected base directory (using safe directory prefix checks).
