...
## 2026-07-25 - Unchecked Cryptographic Randomness Generation
**Vulnerability:** The application generated a new encryption key using `SecRandomCopyBytes` but discarded the returned `OSStatus` with `_ =`, meaning failures (e.g., entropy exhaustion) would silently result in an all-zero or predictable key.
**Learning:** System cryptographic APIs often return status codes rather than throwing errors. Ignoring these codes can lead to silent bypasses of security controls, leaving the application in a highly vulnerable but operational state.
**Prevention:** Always verify the return code of `SecRandomCopyBytes` (and similar APIs) against `errSecSuccess` and handle failures explicitly by throwing domain-specific errors (like `SecurityError.encryptionFailed`) to halt operations.
