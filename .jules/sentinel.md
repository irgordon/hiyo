...
## 2026-05-18 - Unchecked Cryptographic API Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ = ` during encryption key generation in `HiyoStore.swift`. If the random number generator failed, it would silently result in a zeroed or uninitialized encryption key.
**Learning:** System security APIs often return an `OSStatus` rather than throwing Swift errors or crashing on failure. Discarding these results can lead to silent, critical security failures.
**Prevention:** Always explicitly check the return value of security APIs like `SecRandomCopyBytes` against `errSecSuccess` and throw a domain-specific error if they fail. Never use the `_ =` operator on these calls.
