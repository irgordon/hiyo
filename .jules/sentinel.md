...
## 2026-07-19 - Unchecked SecRandomCopyBytes Return Value
**Vulnerability:** In `HiyoStore.swift`, `SecRandomCopyBytes` was used to generate an encryption key, but its return value (`OSStatus`) was discarded using `_ =`. If random generation failed, the key data would silently remain zeroes, resulting in a completely predictable encryption key.
**Learning:** System cryptographic functions that populate buffers do not throw Swift errors; they return status codes. Failing to check these codes leads to silent security failures.
**Prevention:** Always verify the `OSStatus` returned by `SecRandomCopyBytes` is `errSecSuccess` and handle failures explicitly by throwing a domain-specific error.
