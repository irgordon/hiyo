...
## 2026-06-15 - Ineffective Symlink Sandbox Escape Prevention
**Vulnerability:** In `SecureMLX.swift`, `resolvingSymlinksInPath()` was called on the path before checking for the `isSymbolicLinkKey` attribute. Because `resolvingSymlinksInPath()` resolves and removes symlinks from the path, the subsequent check for symbolic links always evaluated to false, rendering the sandbox escape prevention ineffective.
**Learning:** Calling `resolvingSymlinksInPath()` resolves the symlinks, modifying the path and removing the evidence of symlinks. Security checks must be performed on the unresolved path components.
**Prevention:** Always check unresolved path components for symlinks first, then resolve the path, and finally verify that the resolved path remains contained within the intended resolved base directory.
