...
## 2026-08-01 - Silent Cryptographic Failures via Unchecked SecRandomCopyBytes
**Vulnerability:** Unchecked OSStatus result from SecRandomCopyBytes in HiyoStore.swift.
**Learning:** Discarding the return value of system cryptographic functions like SecRandomCopyBytes using `_ =` can lead to silently using zeroed memory if random byte generation fails (e.g., entropy issues).
**Prevention:** System cryptographic APIs returning OSStatus must be explicitly verified against errSecSuccess and handle failures appropriately instead of discarding them.
