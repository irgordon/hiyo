...
## 2026-05-20 - Unchecked Cryptographic Randomness
**Vulnerability:** `SecRandomCopyBytes` was called with its `OSStatus` return value ignored using `_ =`. If random generation failed, the key data remained zeroed, leading to silent encryption with a predictable zero-key.
**Learning:** System cryptographic APIs in Swift often return an error code instead of throwing. Discarding this return value with `_ =` masks critical failures.
**Prevention:** Always verify the returned `OSStatus` from `SecRandomCopyBytes` (and similar APIs) is `errSecSuccess` and handle failures explicitly by throwing or aborting.
