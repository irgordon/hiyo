...
## 2026-07-23 - Unchecked Cryptographic RNG
**Vulnerability:** The application generated a symmetric encryption key using `SecRandomCopyBytes` but ignored its return value with the `_ =` operator. If the random number generator failed, the key buffer would remain initialized to zeroes, resulting in a completely predictable encryption key.
**Learning:** System cryptographic APIs can fail. Discarding their return values silently bypasses security guarantees and can lead to catastrophic vulnerabilities like zeroed keys.
**Prevention:** Always explicitly verify the returned `OSStatus` of cryptographic functions (like checking for `errSecSuccess`) and throw an appropriate error to halt the operation if key generation fails.
