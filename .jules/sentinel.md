...
## 2026-06-15 - Unchecked Cryptographic Result
**Vulnerability:** The application ignored the `OSStatus` return value of `SecRandomCopyBytes` when generating an encryption key, masking random generation failures and potentially resulting in a zero-filled key.
**Learning:** System cryptographic APIs must be checked for success (`errSecSuccess`). Discarding the result can cause silent security bypasses (e.g. all-zero keys).
**Prevention:** Always verify the returned `OSStatus` from `SecRandomCopyBytes` and explicitly throw a domain-specific error (like `SecurityError.encryptionFailed`) on failure rather than proceeding.
