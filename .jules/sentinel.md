...
## 2026-06-01 - Unchecked Cryptographic Random Number Generation
**Vulnerability:** The application used `SecRandomCopyBytes` to generate an encryption key but discarded the return `OSStatus` using `_ =`. If random generation failed, the buffer would remain zeroed, leading to a silent security bypass where data is encrypted with an all-zero key.
**Learning:** System cryptographic APIs can fail due to resource exhaustion or missing entitlements. Discarding their return values masks critical failures that compromise the integrity of cryptographic operations.
**Prevention:** Never discard results from cryptographic APIs. Always explicitly check for `errSecSuccess` and throw a domain-specific error (like `SecurityError.encryptionFailed`) on failure to prevent insecure states.
