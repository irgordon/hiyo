...
## 2026-06-15 - Discarded Cryptographic Status Codes
**Vulnerability:** The application ignored the `OSStatus` return value of `SecRandomCopyBytes` during encryption key generation. If the RNG failed, it could silently fall back to an all-zero key, leading to a cryptographic bypass.
**Learning:** System cryptographic APIs in Swift often return status codes rather than throwing errors. Using `_ =` to silence unused result warnings can inadvertently mask critical security failures.
**Prevention:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded via the `_ =` operator. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and throw a domain-specific error (like `SecurityError.encryptionFailed`) on failure.
