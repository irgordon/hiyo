...
## 2026-08-28 - Discarded Cryptographic Status Codes
**Vulnerability:** System cryptographic APIs like `SecRandomCopyBytes` had their `OSStatus` return values discarded via `_ =`, which could allow silent fallback to zeroed buffers if random generation fails.
**Learning:** Discarding results of security-critical functions breaks fail-secure principles and can lead to silent encryption failures.
**Prevention:** Always capture and verify the `OSStatus` returned by system cryptography APIs, specifically checking for `errSecSuccess` and throwing domain-specific errors on failure.
