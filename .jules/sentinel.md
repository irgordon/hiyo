...
## 2026-08-11 - Unchecked Cryptographic Random Number Generation
**Vulnerability:** The result of `SecRandomCopyBytes` was ignored, which could lead to using a zeroed buffer as an encryption key if the random number generation failed.
**Learning:** System cryptographic and security APIs must never have their results discarded. A failure can silently compromise the security of the application.
**Prevention:** Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately (e.g., by throwing an error).
