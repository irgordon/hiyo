...
## 2026-02-01 - Symlink Path Validation Bypass
**Vulnerability:** Path traversal and sandbox escape via symlinks due to calling `resolvingSymlinksInPath()` before checking `isSymbolicLinkKey`.
**Learning:** `resolvingSymlinksInPath()` eliminates symlinks from the returned path string, completely bypassing subsequent manual symlink checks.
**Prevention:** Always check unresolved path components for symlinks first, then resolve the path and verify containment within the resolved base directory to handle system-level symlinks securely.
