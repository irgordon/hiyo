...
## 2026-07-03 - Discarded Cryptographic API Results
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =`. If random generation failed, the key buffer remained zeroed, leading to predictable encryption keys.
**Learning:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded. They can fail silently (e.g., entropy pool exhaustion), resulting in insecure fallback states like zeroed buffers.
**Prevention:** Always explicitly capture and verify the returned `OSStatus` against `errSecSuccess`, throwing a domain-specific error if the operation fails to prevent insecure states.
