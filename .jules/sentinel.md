...
## 2025-02-18 - Unverified SecRandomCopyBytes Result
**Vulnerability:** The OSStatus result of `SecRandomCopyBytes` was discarded, which could silently result in zeroed keys (e.g. if the system RNG fails/exhausts entropy).
**Learning:** Security APIs often return status codes rather than throwing exceptions, making it easy to accidentally ignore failures via `_ =` assignment.
**Prevention:** Always verify the returned `OSStatus` against `errSecSuccess` and handle failures explicitly by throwing domain-specific errors (e.g., `SecurityError.encryptionFailed`).
