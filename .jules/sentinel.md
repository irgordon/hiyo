...
## 2026-06-15 - Unchecked Cryptographic Randomness
**Vulnerability:** The application used `SecRandomCopyBytes` to generate an encryption key but discarded the `OSStatus` result using `_ =`. If the system random number generator failed, the key buffer would remain initialized to zeros, resulting in a predictable encryption key and a silent security bypass.
**Learning:** Cryptographic API failures do not crash the application by default; they return error codes. Ignoring these codes can lead to catastrophic, silent security failures like zeroed keys.
**Prevention:** System cryptographic and security APIs must never have their results discarded. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and throw an appropriate error to halt operation if key generation fails.
