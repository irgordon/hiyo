...
## 2026-08-01 - Unchecked Cryptographic API Results
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ = `, meaning if the random number generator failed, it would silently proceed with a zero-filled buffer as the encryption key.
**Learning:** System cryptographic APIs return an `OSStatus`. Discarding this status can lead to silent cryptographic failures, such as using completely predictable (zeroed) material for keys.
**Prevention:** Never discard the result of security APIs using `_ =`. Always capture the returned `OSStatus`, explicitly verify it against `errSecSuccess`, and throw an appropriate error or safely abort if the operation fails.
