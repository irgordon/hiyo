...
## 2026-05-18 - Unchecked Cryptographic API Results
**Vulnerability:** System cryptographic and security APIs like `SecRandomCopyBytes` had their results discarded via the `_ =` operator.
**Learning:** Discarding the result of `SecRandomCopyBytes` is a critical vulnerability. If the operation fails, it leaves the memory buffer initialized with zeroes, leading to the use of a zeroed encryption key that is saved to the keychain.
**Prevention:** Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately to prevent silent security bypasses (like zeroed encryption keys).
