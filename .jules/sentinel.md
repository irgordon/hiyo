...
## 2026-06-22 - Unhandled Cryptographic RNG Failures
**Vulnerability:** The application generated encryption keys using `SecRandomCopyBytes` but ignored the returned `OSStatus` using `_ =`. If random generation failed, an all-zero or uninitialized key was used silently.
**Learning:** System cryptographic APIs must have their results verified. Discarding error codes from secure operations can lead to silent but catastrophic security bypasses.
**Prevention:** Always explicitly check the `OSStatus` of functions like `SecRandomCopyBytes` and ensure it equals `errSecSuccess`. Throw an appropriate error on failure.
