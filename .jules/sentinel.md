...
## 2026-06-03 - Ineffective Symlink Validation Due to Pre-Resolution
**Vulnerability:** In `SecureMLX.swift`, symlink validation used `let resolved = cache.resolvingSymlinksInPath()` before checking `isSymbolicLinkKey`. This neutralized the check because `resolvingSymlinksInPath()` replaces all symlinks with their real paths, making the subsequent `isSymbolicLinkKey` check always false.
**Learning:** Resolving a path before validating it for symlinks destroys the evidence of the symlinks, rendering the validation ineffective against symlink-based sandbox escape attempts.
**Prevention:** Always perform symlink checks on the unresolved path components. Then, resolve the path and verify it remains contained within the intended base directory.
