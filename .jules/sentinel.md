...
## 2026-06-13 - Silent Cryptographic Failure Bypass
**Vulnerability:** In `HiyoStore.swift`, the `OSStatus` result of `SecRandomCopyBytes` was discarded using `_ =`. If random byte generation failed, it would silently return a zeroed `Data` buffer, resulting in a predictable all-zero encryption key.
**Learning:** System security APIs in C/Objective-C do not throw Swift errors; they return status codes. Discarding these codes bypasses the failure mechanism and leads to insecure default states (like zeroed memory) being treated as valid cryptographically secure data.
**Prevention:** System cryptographic APIs must never have their results discarded. Always capture the returned `OSStatus`, explicitly verify it equals `errSecSuccess`, and throw a domain-specific error on failure to ensure fail-secure behavior.
