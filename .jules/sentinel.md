...
## 2026-02-01 - Predictable Encryption Keys via Ignored OSStatus
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` during symmetric encryption key generation in `HiyoStore`. If key generation failed (e.g., lack of entropy), it would silently use zeroed memory as the encryption key, defeating encryption entirely.
**Learning:** In Swift, unlike throwing functions, C-based security APIs like `SecRandomCopyBytes` return an `OSStatus` code. Developers accustomed to Swift's error handling might use `_ =` to suppress "result unused" warnings, inadvertently introducing critical cryptographic failures.
**Prevention:** System cryptographic and security APIs returning `OSStatus` must never have their results discarded. Always verify the status equals `errSecSuccess` and handle failures explicitly by throwing domain-specific errors (like `SecurityError.encryptionFailed`).
