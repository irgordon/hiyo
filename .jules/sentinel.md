...
## 2026-07-06 - Unhandled Cryptographic API Failures
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using the `_ =` operator during encryption key generation in `HiyoStore`. If random generation failed, it could result in an all-zero key, silently bypassing encryption.
**Learning:** System cryptographic APIs like `SecRandomCopyBytes` can fail. Discarding their return status creates silent security vulnerabilities where operations appear to succeed but leave data unprotected.
**Prevention:** Always capture the `OSStatus` returned by cryptographic APIs, explicitly check it against `errSecSuccess`, and throw domain-specific errors on failure to ensure secure termination.
