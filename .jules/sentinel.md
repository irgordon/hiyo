...
## 2026-06-01 - Unchecked OSStatus in Security Operations
**Vulnerability:** System cryptographic and security APIs (like `SecRandomCopyBytes`) had their results discarded via the `_ =` operator. If random generation failed, it would silently return a completely zeroed encryption key, bypassing cryptographic security.
**Learning:** System APIs returning `OSStatus` are critical indicators of success or failure. Ignoring them can lead to severe but silent security bypasses.
