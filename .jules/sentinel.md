...
## 2024-07-18 - Useless Symlink Check Due to Pre-Resolution
**Vulnerability:** `secureCacheDirectory()` in `SecureMLX.swift` checked for symlinks on a path *after* calling `resolvingSymlinksInPath()`. This function removes all symlinks and replaces them with their physical target paths, meaning `isSymbolicLinkKey` would always be false. Furthermore, the resolved path was never checked to ensure it remained within the base directory, allowing a sandbox escape.
**Learning:** Resolving symlinks before checking for their existence defeats the purpose of the check. Additionally, the resolved path must also be validated to ensure it hasn't escaped the base directory, as the base directory itself might be a symlink.
**Prevention:** Always check for symlinks on the unresolved path components. Then, resolve both the target path and the intended base directory, and strictly verify containment using exact match or trailing slash prefix.
