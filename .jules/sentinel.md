...
## 2024-06-29 - Symlink Path Validation Bypass
**Vulnerability:** In `SecureMLX.swift`, the `secureCacheDirectory()` method called `resolvingSymlinksInPath()` before checking the URL's `isSymbolicLinkKey` attribute. Because `resolvingSymlinksInPath()` resolves all symlinks, the resulting URL never reports being a symlink, silently bypassing the sandbox escape check.
**Learning:** Checking for symlink presence on a path that has already been resolved for symlinks is fundamentally flawed and renders the security check completely ineffective.
**Prevention:** Always perform symlink detection on the raw, unresolved path components first. Once verified, resolve the path and perform a final containment check against the resolved base directory to ensure no escapes occurred.
