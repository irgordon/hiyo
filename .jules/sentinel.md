...
## 2026-08-29 - Unchecked Cryptographic API Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ = ` when generating an encryption key. If the RNG failed, it would silently result in an all-zero key, severely compromising encryption at rest.
**Learning:** System cryptographic APIs in Swift often return `OSStatus` instead of throwing errors. Discarding their results can lead to silent, catastrophic security failures.
**Prevention:** Never discard the result of cryptographic operations. Always explicitly verify that the returned `OSStatus` equals `errSecSuccess` and handle failures securely.
