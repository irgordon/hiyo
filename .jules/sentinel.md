...
## 2026-06-18 - Unchecked Random Byte Generation
**Vulnerability:** The result of `SecRandomCopyBytes` was explicitly ignored using `_ =` when generating the database encryption key in `HiyoStore.swift`. If the random number generator failed, the key data buffer would remain zeroed, resulting in a completely predictable all-zero encryption key.
**Learning:** Security and cryptographic APIs that return a status code can fail. Ignoring these return values can silently compromise the security of the entire system by allowing the application to proceed in a degraded, insecure state (like using an empty key).
**Prevention:** System cryptographic and security APIs must never have their results discarded. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures securely (e.g., by throwing an error or aborting).
