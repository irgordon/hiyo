...
## 2026-06-12 - Ineffective Symlink Validation Due to Pre-Resolution
**Vulnerability:** Symlink checks for sandbox escape prevention were performed *after* calling `resolvingSymlinksInPath()`. Because the method resolves and removes symlinks from the path string, the subsequent check for `.isSymbolicLinkKey` always returned false, rendering the security control useless.
**Learning:** Checking for symlinks on an already resolved path defeats the purpose of the check. Additionally, comparing resolved paths against unresolved base directories can cause false positives since system directories (like `/var`) are often symlinks themselves.
**Prevention:** Always check for symlinks on the unresolved path components. Then, resolve both the target path and the base directory before performing containment validation (prefix/equality checks).
