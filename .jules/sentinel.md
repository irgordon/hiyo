...
## 2026-06-07 - Ignored OSStatus from SecRandomCopyBytes
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using the `_ =` operator. If random byte generation failed, it would silently leave the data buffer zeroed out, resulting in a cryptographically weak (all-zero) encryption key.
**Learning:** System cryptographic APIs like `SecRandomCopyBytes` return `OSStatus` to indicate success or failure. Discarding this status can lead to silent failures and severe security bypasses.
**Prevention:** System cryptographic and security APIs must never have their results discarded via the `_ =` operator. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately (e.g., throw an error).
