...
## 2026-06-25 - Ineffective Symlink Verification due to Path Resolution
**Vulnerability:** The `SecureMLX.swift` cache directory validation was calling `resolvingSymlinksInPath()` before checking for the `isSymbolicLinkKey` attribute on path components. Because resolving the path inherently removes any symlinks, the subsequent check was rendered useless and would fail to detect symlink-based sandbox escapes.
**Learning:** Checking for symlinks on a path that has already been resolved is ineffective since the resolution process follows the symlink and returns the target path's metadata, hiding the existence of the link.
**Prevention:** Always verify the existence of symlinks using the unresolved path components first. Afterward, resolve the path and enforce containment within the intended base directory bounds.
