...
## 2026-05-15 - Unhandled OSStatus in Cryptographic API
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded (`_ =`), silently allowing an all-zero key to be used for encryption if random byte generation failed.
**Learning:** System cryptographic and security APIs must never have their results discarded. A failure during key generation could silently bypass encryption entirely.
**Prevention:** Always explicitly verify the returned `OSStatus` (e.g., `errSecSuccess`) and handle failures appropriately (e.g., throwing a `SecurityError`) to prevent insecure states.
