...
## 2026-06-11 - Ignored Cryptographic API Results
**Vulnerability:** The application used `SecRandomCopyBytes` to generate an encryption key in `HiyoStore.swift` but discarded its return value (`_ = `). If the RNG failed, it would silently result in a zeroed, completely predictable encryption key.
**Learning:** System cryptographic APIs in C/Objective-C style return status codes instead of throwing Swift errors. Discarding these results masks critical failures.
**Prevention:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately (e.g., throwing a domain error) to prevent insecure states.
