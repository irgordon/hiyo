...
## 2026-06-05 - Flawed Symlink Check Due to Pre-Resolution
**Vulnerability:** Symlink checks for sandbox escape prevention were performed on a path that had already been resolved using `resolvingSymlinksInPath()`. This inherently removes all symlinks from the path string, causing the `isSymbolicLinkKey` check to always evaluate to false, rendering the security control completely ineffective.
**Learning:** Checking for symlinks on a resolved path is useless. The path components must be checked for symlinks before resolution.
**Prevention:** Always check unresolved path components for the `isSymbolicLinkKey` attribute. After ensuring no malicious symlinks exist in the components, resolve the path and verify it remains contained within the intended resolved base directory.
