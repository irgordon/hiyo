...
## 2026-05-18 - Unchecked Cryptographic Random Number Generation
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ = ` when generating the storage encryption key. If random number generation fails, the application would proceed to use a predictable (e.g., all-zeroes) key for encryption.
**Learning:** Cryptographic API failures can silently bypass security controls if their return codes are not explicitly checked. Discarding results for these calls assumes infallibility where environmental constraints might cause failures.
**Prevention:** Always verify the returned `OSStatus` from cryptographic system APIs (like `errSecSuccess` for `SecRandomCopyBytes`) and throw a domain-specific error if the operation fails.
