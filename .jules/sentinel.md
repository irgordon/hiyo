...
## 2026-06-25 - Useless Symlink Validation
**Vulnerability:** Calling `resolvingSymlinksInPath()` on a URL before checking for `isSymbolicLinkKey` removes the symlinks, making the subsequent symlink validation check useless and allowing sandbox escapes.
**Learning:** Symlinks are removed when a path is resolved. Validating a resolved path for symlink attributes will always return false, creating a false sense of security.
**Prevention:** Always check unresolved path components for symlinks first, then resolve the path and verify it remains contained within the intended base directory (after also resolving the base directory itself).
