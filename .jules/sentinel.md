...
## 2026-06-25 - Discarded Cryptographic API Results
**Vulnerability:** In `HiyoStore.swift`, the application used `SecRandomCopyBytes` to generate a random encryption key but intentionally ignored the return value (`_ = ...`). If the random number generator failed, the system silently continued to use a zeroed-out, highly predictable key, completely bypassing encryption-at-rest.
**Learning:** Security APIs often return status codes that must be checked. System resource exhaustion, RNG failure, or entropy issues can cause crypto generation to fail, leaving initialized zero-state buffers intact.
