...
## 2026-06-23 - Unchecked Random Number Generation
**Vulnerability:** The result of `SecRandomCopyBytes` was being explicitly ignored using `_ =` during encryption key generation. If the entropy source failed, the key data buffer would remain zeroed, resulting in a predictable symmetric encryption key.
**Learning:** Cryptographic system functions that can fail must always have their return values checked. Ignoring the return value of a secure random generator can silently convert a secure process into a deterministically vulnerable one without crashing the application.
**Prevention:** Always verify the `OSStatus` returned by `SecRandomCopyBytes` equals `errSecSuccess` and handle the error explicitly (e.g., throwing a domain error) to fail securely.
