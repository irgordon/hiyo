...
## 2026-06-21 - Ineffective Symlink Validation via Premature Resolution
**Vulnerability:** Symlink checks in `SecureMLX.swift` verified the presence of `isSymbolicLinkKey` *after* calling `resolvingSymlinksInPath()`. Because resolved paths have no symlinks, the validation was bypassed. Additionally, if the path was resolved to a directory outside the sandbox, the check failed to ensure the final path remained within the allowed bounds.
**Learning:** Checking for symlinks on a path that has already been resolved is inherently ineffective. Sandbox escapes can occur if both the unresolved components aren't verified AND the final resolved destination isn't bounded to the intended directory.
**Prevention:** When validating paths to prevent symlink-based sandbox escapes, check the unresolved path components for symlinks, then resolve the path and verify it remains contained within the intended base directory (using prefix or equality bounds checking).
