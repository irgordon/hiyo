...
## 2026-06-15 - Futile Symlink Validation on Resolved Paths
**Vulnerability:** Symlink detection was performed on a path *after* calling `resolvingSymlinksInPath()`. Since the method removes symlinks, the check was useless and could allow an attacker to bypass sandbox limits via a symlinked cache directory. Additionally, the resolved path was not verified against a resolved base directory.
**Learning:** Checking for `.isSymbolicLinkKey` on a path that has already been resolved will always return false because the symlink has been followed and removed from the path string.
**Prevention:** Always check for symlinks on the unresolved path components first. Afterwards, resolve the path and verify it still resides within the intended resolved base directory.
