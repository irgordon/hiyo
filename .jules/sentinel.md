...
## 2026-08-26 - Unchecked SecRandomCopyBytes Result
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` when generating an encryption key in `HiyoStore.swift`. If random number generation fails, this leaves the buffer zeroed out, resulting in the silent use of a completely insecure, all-zero symmetric key.
**Learning:** System cryptographic APIs in Apple platforms typically signal failure through return codes rather than throwing exceptions or trapping. Discarding these results can lead to silent but catastrophic security failures.
**Prevention:** System cryptographic and security APIs (like `SecRandomCopyBytes`) must never have their results discarded. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and handle failures appropriately (e.g., throwing a domain-specific error).
