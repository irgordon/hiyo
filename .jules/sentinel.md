...
## 2026-08-31 - Useless Symlink Validation Due to Early Resolution
**Vulnerability:** The application checked for symbolic links in path components after calling `resolvingSymlinksInPath()`, which removes symlinks before the check occurs, rendering the sandbox escape protection completely ineffective.
**Learning:** Validating properties of a path (like checking for symlinks) must be performed on the raw, unresolved path components, otherwise the very elements you are trying to detect will be normalized away.
**Prevention:** Check unresolved path components for symlinks first, then resolve the path and verify it remains contained within the intended base directory (after also resolving the base directory).
