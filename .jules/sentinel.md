...
## 2026-05-18 - Ignored OSStatus from SecRandomCopyBytes
**Vulnerability:** Ignoring the result of `SecRandomCopyBytes` by assigning it to `_` can lead to using uninitialized (zeroed) memory for cryptographic keys if the random number generator fails.
**Learning:** System cryptographic and security APIs must have their results checked. If `SecRandomCopyBytes` fails, it does not populate the buffer, leaving it with its initial contents (often zeros).
**Prevention:** Always check the `OSStatus` result from `SecRandomCopyBytes` and fail securely (e.g., throwing an error) if it is not `errSecSuccess`.
