...
## 2024-05-24 - Unchecked Cryptographic API Results
**Vulnerability:** The result of `SecRandomCopyBytes` was discarded using `_ =` in `HiyoStore.swift`. If random byte generation failed, the application would silently proceed with a zeroed-out encryption key, severely compromising data at rest.
**Learning:** System cryptographic APIs in Swift often return an `OSStatus` rather than throwing errors. Discarding this return value suppresses critical failure states.
**Prevention:** Always explicitly check the `OSStatus` of Security framework APIs (e.g., `guard status == errSecSuccess else { throw ... }`) and handle failures securely rather than ignoring them or using `fatalError`.
