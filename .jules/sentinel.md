...
## 2026-05-18 - Unchecked Cryptographic Randomness Error Status
**Vulnerability:** The codebase invoked `SecRandomCopyBytes` to generate cryptographic keys, but explicitly discarded the return value using `_ =`. If random generation failed (e.g., entropy exhaustion), it would leave the initial zeroed `Data` buffer unmodified, leading to the creation of easily predictable, all-zero symmetric keys.
**Learning:** System cryptographic APIs often communicate failures through their return types (like `OSStatus`). Ignoring these status codes allows the application to silently continue execution using compromised or zeroed cryptographic material.
**Prevention:** Never discard the result of a system cryptographic API (like `SecRandomCopyBytes`). Always capture the returned `OSStatus`, verify that it equals `errSecSuccess`, and throw a designated error (e.g., `SecurityError.encryptionFailed`) or halt execution securely upon failure to prevent the application from entering an insecure state.
