...
## 2024-05-24 - Unhandled System Cryptographic APIs
**Vulnerability:** System cryptographic and security APIs (like `SecRandomCopyBytes`) had their returns discarded or non-fatally handled, potentially allowing execution to continue with zeroed/predictable key buffers if the system ran out of entropy or otherwise failed.
**Learning:** Returning `OSStatus` is a critical control path in Apple security APIs. If randomness generation fails, falling back to a thrown error (which could be caught and ignored) or simply discarding the return with `_ =` means the application operates with an insecure key (e.g. all 0s).
**Prevention:** Always verify `OSStatus == errSecSuccess`. For critical key generation failures, use `fatalError()` to safely crash the application and prevent it from entering an insecure state.
