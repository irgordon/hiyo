...
## 2026-06-25 - Ignored Cryptographic Return Values
**Vulnerability:** The result of `SecRandomCopyBytes` during encryption key generation was discarded using `_ =`. If random byte generation failed (e.g., due to system state), it would silently proceed with a zeroed `Data` buffer, resulting in all encrypted data using an insecure, predictable all-zero key.
**Learning:** System cryptographic APIs often return status codes rather than throwing Swift errors. Discarding these results can lead to catastrophic, silent security failures like zeroed keys.
**Prevention:** Never discard the result of cryptographic functions like `SecRandomCopyBytes`. Always explicitly verify the returned `OSStatus` is `errSecSuccess` and throw domain-specific errors if it fails.
