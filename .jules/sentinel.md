...
## 2026-05-20 - Unchecked Cryptographic API Results
**Vulnerability:** The application used `SecRandomCopyBytes` to generate an encryption key in `HiyoStore.swift` but explicitly ignored the returned `OSStatus` using `_ =`. If random generation failed, it would silently result in an all-zero encryption key.
**Learning:** Cryptographic functions often return a status code rather than throwing. Ignoring these results can lead to catastrophic security failures, such as using uninitialized (zeroed) memory for cryptographic keys.
**Prevention:** Always verify the returned `OSStatus` of system security APIs (like ensuring it equals `errSecSuccess`) and throw an error or handle the failure appropriately if the operation does not succeed.
